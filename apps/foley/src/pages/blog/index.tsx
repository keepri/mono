    ...
    return (
        <section className="flex flex-col items-center gap-4 min-h-screen bg-ivory dark:bg-black">
            <header className="text-center dark:text-white">
                <Bounce
                    enabled={!!errMessage.length || alertSignIn}
                    className={`${
                        errMessage.length || alertSignIn ? "" : "invisible"
                    } mb-2`}
                >
                    <p className="text-lg">
                        {errMessage.length ? errMessage : "please sign in"}
                    </p>
                </Bounce>

                <h1
                    className={`max-xs:text-8xl text-[200px] leading-none ${fontLondrinaSketch}`}
                >
                    blog
                </h1>
            ...
            <ul
                className={`${
                    props.posts.length
                        ? "flex flex-col gap-6 mt-10 text-white"
                        : "hidden"
                }`}
            >
                {props.posts.map((post) => (
                    ...
                    >
                        <h4 className="text-2xl font-medium group-hover:text-[var(--clr-orange)] group-hover:underline">
                            {post.title}
                        </h4>
...
export async function getStaticProps(
    ...
    GetStaticPropsResult<{
        posts: Array<
            Pick<
                Post,
                "title" | "description" | "excerpt" | "slug" | "category"
            > & { createdAt: string }
        >;
    }>
> {
...
function memoizePostSlugs(posts: Props["posts"]): Set<Slug> | undefined {
    ...
    return useMemo(
        () =>
            posts.length === 0
                ? void 0
                : new Set<Slug>(posts.map((post) => post.slug)),
        []
    );
}
